...
knn_fit <- knn3(population_tf=="True" ~ hc + rgdpna, data=pwt)
...
fit<-lm(rgdpna~.,data=pwt_reg) #predict rgdpna from all other variables
yhat <- predict(fit)
...
data<-pwt[sample(nrow(pwt)),] #randomly order rows
folds<-cut(seq(1:nrow(pwt)),breaks=k,labels=F) #create folds
diags<-NULL
for(i in 1:k){
  train<-data[folds!=i,]
  test<-data[folds==i,]
